...
if [[ -n "$profile" ]]; then
  printf '[gcenv] GCP profile isolation is active in this Claude Code session. Invoke it as `gcenv` (it is on PATH); never hardcode a versioned cache path like .../gcenv/<ver>/bin/gcenv.\n\nActive profile: %s%s%s.\ngcloud, bq, gsutil, terraform, kubectl, and helm are auto-scoped to this profile by a PreToolUse hook — run them normally; no wrapping needed.%s\n\nRules: switch with `gcenv claude use <name>` (never `gcloud config set`). Never run *raw* `gcloud auth login` / `gcloud auth application-default login` / `gcloud config set` here — they mutate global machine-wide state; use gcenv. To (re)authenticate, run `gcenv login %s` with the Bash tool timeout at 600000: on a machine with a browser this does loopback OAuth in-session (gcloud opens your browser or prints a URL to click, and reads the code from a localhost callback — no TTY needed). Confirm with the user first — it opens a browser and overwrites the global default ADC. If `gcenv` errors with `command not found: _gcenv_*`, rerun as `command gcenv ...`. Full playbook: the `gcenv` skill.\n' \
